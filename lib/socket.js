...
function socketConnect (server) {
	...
	io.on('connection', function (socket) {
		...
		socket.on('submitResult', function (reqParams) {
	        ...
	        var raidObj = {
	        	_id: raidId,
	            _matchId: socket.matchId,
	            _resultId: reqParams._resultId,
	        ...
	        io.sockets.in(socket.room).emit(socket.matchId, raidObj);  //emit message to all members of the room only
	        //msgObj.isRead = true;
