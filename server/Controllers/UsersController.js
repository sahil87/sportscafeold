...
exports.postUserTest = function(req, res) {
    //console.log(req.body);
    var randKey = Math.random().toString(36).slice(2);
    var params = {
            "email": "isports" + randKey + "@yahoo.com",
            "public_details": {
                "age": 25,
                "first_name": "Test",
                "last_name": "User",
                "gender": "M",
                "birthday": "1990-02-05T22:00:00.000Z",
                "bio": "Hi there",
                "about": "",
                "location": [
                    51.98169291849577,
    ...
    checkNPostUser(params, function(err, user) {
        if(err) {
            console.log(err);
            console.log("Found problem, UsersController L360");
            res.status(500).json(err);
        } else {            
            res.status(201).json(user);
        }
